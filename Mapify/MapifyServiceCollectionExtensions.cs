...
public static class MapifyServiceCollectionExtensions {
    ...
    public static IServiceCollection AddMapify(this IServiceCollection services, ServiceLifetime lifecycle = ServiceLifetime.Singleton) {
        ...
        if (!services.Any(x => x.ServiceType == typeof(IMapify))) {
            var descriptor = new ServiceDescriptor(
                typeof(IMapify),
                sp => {
                    var registrations = sp.GetServices<MapifyProfileTypeRegistration>()
                        .Where(x => x.MapperName == null)
                        .Select(x => (MapifyProfile)sp.GetRequiredService(x.ProfileType))
                        .ToArray();

                    return new Mapify(registrations);
                },
                lifecycle
            );
    ...
    public static IServiceCollection AddMapifyNamed(this IServiceCollection services, string name, ServiceLifetime lifecycle = ServiceLifetime.Singleton) {
        ...
        if (!services.Any(x => x.ServiceType == typeof(NamedMapifyRegistration)
            && x.ImplementationInstance is NamedMapifyRegistration existing
            && string.Equals(existing.Name, name, StringComparison.Ordinal))) {
            services.AddSingleton(new NamedMapifyRegistration(name, lifecycle));
            ...
            var descriptor = new ServiceDescriptor(
                ...
                sp => {
                    ...
                    var mapper = new Mapify(registrations);
                    return new NamedMapify(name, mapper);
                },
                lifecycle
            );
    ...
    public static IServiceCollection AddMapifyProfiles(this IServiceCollection services, string? mapperName, params Assembly[] profileAssemblies) {
        ...
        foreach (var assembly in profileAssemblies.Distinct()) {
            foreach (var type in GetLoadableTypes(assembly)) {
                if (!typeof(MapifyProfile).IsAssignableFrom(type) || type.IsAbstract || type.IsInterface) {
                    continue;
                }
    ...
    public static IMapify GetMapify(this IServiceProvider serviceProvider, string name) {
        ...
        var named = serviceProvider.GetServices<INamedMapify>()
            .SingleOrDefault(x => string.Equals(x.Name, name, StringComparison.Ordinal));

        return named?.Mapper ?? throw new InvalidOperationException($"No named Mapify mapper registered with name '{name}'."); ;
    }

    private static void RegisterProfileType(IServiceCollection services, string? mapperName, Type type) {
        if (services.Any(d => d.ServiceType == typeof(MapifyProfileTypeRegistration)
            && d.ImplementationInstance is MapifyProfileTypeRegistration existing
        ...
        services.AddSingleton(new MapifyProfileTypeRegistration(mapperName, type));
    }
